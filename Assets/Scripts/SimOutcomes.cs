...
public class SimOutcomes : MonoBehaviour
{
    ...
    public void CheckOutcomes(CarrotSim carrot, string province, bool useChemical, bool useCompost, EduPopup popup)
    {
        ...
        {
            popup.Show("WEEDS TOOK OVER!", "Chemical fertiliser alone makes weeds explode. Organic compost makes plants more pest-resistant. Your crop is choked.");
            carrot.SetStage(7);
            carrot.plantHealth = 0f;
            return;
        }

        if (!useChemical && !useCompost && carrot.fertiliser <= 10f)
        {
            popup.Show("LOW NUTRIENTS", "Your soil is running out of nutrients. Add fertiliser or compost to keep plants healthy.");
